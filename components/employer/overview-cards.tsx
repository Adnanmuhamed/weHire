/**
 * Overview Cards Component
 * 
 * Displays key metrics in card format.
 * Stateless presentational component.
 */

interface OverviewCardsProps {
  totalJobs: number;
  openJobs: number;
  totalApplications: number;
}

export default function OverviewCards({
  totalJobs,
  openJobs,
  totalApplications,
}: OverviewCardsProps) {
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
      {/* Total Jobs Card */}
      <div className="p-6 border border-foreground/10 rounded-lg bg-background">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm text-foreground/60 mb-1">Total Jobs</p>
            <p className="text-2xl font-semibold text-foreground">{totalJobs}</p>
          </div>
          <div className="p-3 bg-foreground/5 rounded-lg">
            <svg
              className="w-6 h-6 text-foreground/60"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
              aria-hidden="true"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth={2}
                d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"
              />
            </svg>
          </div>
        </div>
      </div>

      {/* Open Jobs Card */}
      <div className="p-6 border border-foreground/10 rounded-lg bg-background">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm text-foreground/60 mb-1">Open Jobs</p>
            <p className="text-2xl font-semibold text-foreground">{openJobs}</p>
          </div>
          <div className="p-3 bg-foreground/5 rounded-lg">
            <svg
              className="w-6 h-6 text-foreground/60"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
              aria-hidden="true"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth={2}
                d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"
              />
            </svg>
          </div>
        </div>
      </div>

      {/* Total Applications Card */}
      <div className="p-6 border border-foreground/10 rounded-lg bg-background">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm text-foreground/60 mb-1">Total Applications</p>
            <p className="text-2xl font-semibold text-foreground">{totalApplications}</p>
          </div>
          <div className="p-3 bg-foreground/5 rounded-lg">
            <svg
              className="w-6 h-6 text-foreground/60"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
              aria-hidden="true"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth={2}
                d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"
              />
            </svg>
          </div>
        </div>
      </div>
    </div>
  );
}

