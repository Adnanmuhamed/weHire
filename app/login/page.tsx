 ...
 * Reads searchParams (role, redirect) to customize Employer vs Candidate experience.
 * 
 * Redirect logic:
...
export default async function LoginPage({
  ...
  const roleValue = roleParam 
    ? (typeof roleParam === 'string' ? roleParam : roleParam[0] || null)
    : null;

  const isEmployerPortal = roleValue === 'EMPLOYER';
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] grid grid-cols-1 md:grid-cols-2">
        ...
        {isEmployerPortal ? (
          <>
            ...
            <blockquote className="mt-12 pl-4 border-l-4 border-foreground/30 text-foreground/80 italic">
              &ldquo;JobPortal helped us find qualified candidates 3x faster than traditional hiring.&rdquo;
            </blockquote>
          </>
        ) : (
          <>
      ...
      <div className="flex flex-col justify-center px-6 sm:px-12 lg:px-16 py-12">
        <LoginForm redirectParam={redirectValue} roleParam={roleValue} />
      </div>
    </div>
  );
}
